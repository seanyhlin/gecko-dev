...
void
PresentationDeviceManager::NotifyDeviceChange()
{
  nsCOMPtr<nsIObserverService> obs = services::GetObserverService();
  if (obs) {
    obs->NotifyObservers(static_cast<nsIPresentationDeviceManager*>(this),
...
{
  NS_ENSURE_ARG(aDevice);
  NS_ENSURE_ARG(aSessionTransport);

  nsCOMPtr<nsIObserverService> obs = services::GetObserverService();
  NS_ENSURE_TRUE(obs, NS_ERROR_FAILURE);

  nsRefPtr<PresentationSessionRequest> request =
    new PresentationSessionRequest(aDevice, aUrl, aPresentationId, aSessionTransport);
